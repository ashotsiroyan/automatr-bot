...
export class AutomationsService {
    ...
    async stopRunningAutomationByActionId(id: number): Promise<Automation | null>{
    ...
    async createNote(createNoteDto: CreateNoteDto){
        try{
            ...
            if(image){
                ...
                filename += '.jpeg';
                const path = join(__dirname, '../../public', 'screenshots', automationId.toString());
    ...
    async removeFinishedAutomations() {
        try {
            ...
            for(let { id } of finishedAutomations){
                await fs.rm(join(path, id.toString()), { recursive: true })
            }
